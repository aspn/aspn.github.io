...
;(function(window) {
	...
	Segmenter.prototype.options = {
		...
		animation: {
			...
			// translateX: {min: 50, max: 70},
			translateY: {min: 30, max: 50}
		},
		// Callbacks
		...
		positions: [
			...
			{top: 2, left: 2, width: 30, height: 50},
			{top: 10, left: 65, width: 30, height: 50},
			{top: 15, left: 5, width: 65, height: 40},
...

{
	...
	const fireworks = (function() {
		...
		const colors = ['#52E5E7', '#130CB7', '#FFF', '#4285F4', '#4A626E', '#1E2130'];
		...
		canvasEl.addEventListener(tap, (e) => {
			setCanvasSize()
            render.play();
            updateCoords(e);
